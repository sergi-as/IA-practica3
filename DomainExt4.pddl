(define (domain hotel)
  ...
  (:predicates
    ...
    (pendiente ?x - reserva)
    ;para indicar que falta por asignar una habitación
    (pendientehab ?x - habitacion)
  )
  ...
  (:action asigna :parameters (?x - reserva ?y - habitacion)
    ...
    :effect ( and ( assignacion ?x ?y) (not (pendiente ?x)) (not (pendientehab ?y))
      (increase (coste) (* (- (dfi ?x) (dini ?x) ) (- (dfi ?x) (dini ?x))))
      (increase (nplazas) (+ (- (capacidad ?y) (npers ?x)) (- (dfi ?x) (dini ?x) ) )) )
