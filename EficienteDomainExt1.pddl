(define (domain hotel)
  ...
  (:action descarta :parameters (?x - reserva )
    ...
    :effect (and (increase (coste) (*(* (- (dfi ?x) (dini ?x) ) (- (dfi ?x) (dini ?x))) (- (dfi ?x) (dini ?x) ) )) (not (pendiente ?x)) )
  )
  (:action asigna :parameters (?x - reserva ?y - habitacion)
    ...
    :effect ( and (increase (coste) (* (- (dfi ?x) (dini ?x) ) (- (dfi ?x) (dini ?x)))) ( assignacion ?x ?y) (not (pendiente ?x)))
  )
